...
def obtener_vacuna_por_id(id):
    ...
    return jsonify({
        ...
        'fecha_creacion': vacuna.fecha_creacion.isoformat() if vacuna.fecha_creacion else None,
        'fecha_modificacion': vacuna.fecha_modificacion.isoformat() if vacuna.fecha_modificacion else None,
...
@token_requerido(roles_permitidos=['administrador', 'veterinario'])
def crear_vacuna():
    data = request.get_json()
    creador = f"{request.rol}_{request.usuario_id}"
    ...
    nueva_vacuna = Vacuna(
        ...
        id_admin=request.usuario_id if request.rol == 'administrador' else None,
        id_veterinario=request.usuario_id if request.rol == 'veterinario' else data.get('id_veterinario'),
...
def crear_aplicacion():
    ...
    if duplicado:
        return jsonify({'error': 'Ya existe una aplicación de esta vacuna para este cerdo en esa fecha'}), 409
...
def listar_aplicaciones():
    ...
    for app in aplicaciones:
        resultado.append({
            ...
            'descripcion': app.descripcion,
            'fecha_creacion': app.fecha_creacion.isoformat() if app.fecha_creacion else None,
            'fecha_modificacion': app.fecha_modificacion.isoformat() if app.fecha_modificacion else None,
...
def actualizar_aplicacion(id):
    ...
    if 'fecha_aplicacion' in data:
        try:
            nueva_fecha_aplicacion = datetime.strptime(data['fecha_aplicacion'], '%Y-%m-%d').date()
        except ValueError:
            return jsonify({'error': 'Formato de fecha inválido. Use YYYY-MM-DD'}), 400
    ...
    if duplicado:
        return jsonify({'error': 'Ya existe otra aplicación con esta vacuna, cerdo y fecha'}), 409
...
def eliminar_aplicacion(id):
    ...
    if not aplicacion:
        return jsonify({'error': 'Aplicación no encontrada'}), 404
    if aplicacion.id_veterinario != request.usuario_id:
        return jsonify({'error': 'No tienes permiso para eliminar esta aplicación'}), 403
...
def obtener_aplicacion_por_id(id):
    ...
    resultado = {
        ...
        'fecha_aplicacion': aplicacion.fecha_aplicacion.isoformat() if aplicacion.fecha_aplicacion else None,
        'descripcion': aplicacion.descripcion,
        'fecha_creacion': aplicacion.fecha_creacion.isoformat() if aplicacion.fecha_creacion else None,
        'fecha_modificacion': aplicacion.fecha_modificacion.isoformat() if aplicacion.fecha_modificacion else None,
...
# Leer y codificar el logo en base64
    ...
    config = pdfkit.configuration(wkhtmltopdf=r'C:\Users\usuario\Desktop\html\wkhtmltox\bin\wkhtmltopdf.exe')
